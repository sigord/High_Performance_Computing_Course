...
double CalcMatMulTime_My_first(double * A, double * B, double * C, size_t N)
{
    struct timeval start, end;
    double r_time = 0.0;
    size_t i, j, k;
...
double CalcMatMulTime_My_second(double * A, double * B, double * C, size_t N)
{
    struct timeval start, end;
    double r_time = 0.0;
    size_t i, j, k, i2, j2, k2;
    double * leftmul, * rightmul, * finalres;
    int len = (64/sizeof(double));
    ...
    for (i = 0; i < N; i += len)
    {
        ...
        for (j = 0; j < N; j += len)
            for (k = 0; k < N; k += len)
                for (i2 = 0, finalres = &C[dummy + j],
                     leftmul = &A[dummy + k]; i2 < len;
                     ++i2, finalres += N, leftmul += N)
                    for (k2 = 0, rightmul = &B[k * N + j];
                         k2 < len; ++k2, rightmul += N)
                        for (j2 = 0; j2 < len; ++j2)
                            finalres[j2] += leftmul[k2] * rightmul[j2];
    }
...
// kij ordering naive optimization (useless for -O3)
    ...
    // My realization first
    average_runtime = 0.0;
    for(int n=0; n<NRuns; n++)
    {
        runtimes[n]=CalcMatMulTime_My_first(&A[0], &B[0], &C[0], N);
        printf("runtime %lf seconds\n", runtimes[n]);
        average_runtime += runtimes[n]/NRuns;
    }
    printf("average runtime my first realization %lf seconds\n", average_runtime);
    printf("---------------------------------\n");


    // My realization second
    average_runtime = 0.0;
    for(int n=0; n<NRuns; n++)
    {
        runtimes[n]=CalcMatMulTime_My_second(&A[0], &B[0], &C[0], N);
        printf("runtime %lf seconds\n", runtimes[n]);
        average_runtime += runtimes[n]/NRuns;
    }
    printf("average runtime my second realization %lf seconds\n", average_runtime);
    printf("---------------------------------\n");
